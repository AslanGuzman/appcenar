...
## 10. Flujo de Git y creación del repositorio

### 10.1 Inicializar el repositorio local

```bash
git init
git add .
git commit -m "chore: initial project setup (express, mongoose, env config)"
```

### 10.2 Historial de commits sugerido (organiza tu trabajo así)
...
git commit -m "chore: add Docker Compose for containerized MongoDB"

git add src/models
git commit -m "feat: add User, Commerce and CommerceType models"

git add src/middlewares/auth.middleware.js
git commit -m "feat: add JWT authentication and role-based authorization middleware"

git add src/middlewares/upload.middleware.js
git commit -m "feat: add multer middleware for image uploads"

git add src/middlewares/validate.middleware.js src/middlewares/errorHandler.middleware.js
git commit -m "feat: add validation and centralized error handling middlewares"

git add src/services src/utils
git commit -m "feat: add email and token services with response helpers"

git add src/validators/auth.validator.js src/controllers/auth.controller.js src/routes/auth.routes.js src/routes/index.js
git commit -m "feat: implement Auth module (login, register, confirm email, password reset)"

git add src/config/swagger.js
git commit -m "docs: integrate Swagger (OpenAPI) documentation"

git add src/seeders
git commit -m "feat: add default admin and ITBIS configuration seeders"

git add src/seeders/demoData.seeder.js src/scripts/seed.js package.json .env.example
git commit -m "feat: add demo data seeder (commerces, products, clients, deliveries, sample orders)"

git add src/utils/pagination.js
git commit -m "feat: add reusable pagination, search and sorting helper"

git add src/models/Configuration.js src/controllers/configuration.controller.js src/routes/configuration.routes.js src/validators/configuration.validator.js
git commit -m "feat: implement Configurations module (ITBIS)"

git add src/models/CommerceType.js src/controllers/commerceType.controller.js src/routes/commerceType.routes.js src/validators/commerceType.validator.js
git commit -m "feat: implement Commerce Types module with cascade delete"

git add src/controllers/account.controller.js src/routes/account.routes.js src/validators/account.validator.js
git commit -m "feat: implement Account module (get/update authenticated profile)"

git add src/controllers/commerceCatalog.controller.js src/routes/commerceCatalog.routes.js
git commit -m "feat: implement Commerce Catalog module for clients"

git add src/models/Category.js src/models/Product.js src/controllers/category.controller.js src/controllers/product.controller.js src/routes/category.routes.js src/routes/product.routes.js src/validators/category.validator.js src/validators/product.validator.js
git commit -m "feat: implement Categories and Products modules for commerce"

git add src/models/Address.js src/controllers/address.controller.js src/routes/address.routes.js src/validators/address.validator.js
git commit -m "feat: implement Addresses module for clients"

git add src/models/Favorite.js src/controllers/favorite.controller.js src/routes/favorite.routes.js
git commit -m "feat: implement Favorites module for clients"

git add src/models/Order.js src/controllers/order.controller.js src/routes/order.routes.js src/validators/order.validator.js
git commit -m "feat: implement Orders module (client, commerce and delivery flows)"

git add src/controllers/adminDashboard.controller.js src/routes/adminDashboard.routes.js
git commit -m "feat: implement Admin Dashboard metrics"

git add src/controllers/adminUser.controller.js src/routes/adminUser.routes.js src/validators/adminUser.validator.js
git commit -m "feat: implement Admin Users module (clients, deliveries, commerces, admins)"

git add src/routes/index.js
git commit -m "chore: mount all API modules in the main router"

git add package.json public/css/styles.css
git commit -m "chore: add web dependencies (sessions, handlebars, flash) and base styles"

git add src/views/layouts src/views/partials
git commit -m "feat: add Handlebars layouts and shared partials (navbar, flash messages)"

git add src/web/middlewares
git commit -m "feat: add session-based auth and role middlewares for the web app"

git add src/web/controllers/authWeb.controller.js src/web/routes/authWeb.routes.js src/views/auth
git commit -m "feat: implement web Auth module (login, register, activation, password reset)"

git add src/web/controllers/clientWeb.controller.js src/web/routes/clientWeb.routes.js src/views/client
git commit -m "feat: implement web Client module (home, catalog, cart, checkout, orders, addresses, favorites)"

git add src/web/controllers/commerceWeb.controller.js src/web/routes/commerceWeb.routes.js src/views/commerce
git commit -m "feat: implement web Commerce module (orders, categories, products, profile)"

git add src/web/controllers/deliveryWeb.controller.js src/web/routes/deliveryWeb.routes.js src/views/delivery
git commit -m "feat: implement web Delivery module (assigned orders, complete order, profile)"

git add src/web/controllers/adminWeb.controller.js src/web/routes/adminWeb.routes.js src/views/admin
git commit -m "feat: implement web Admin module (dashboard, users, configuration, commerce types)"

git add src/web/routes/index.js src/app.js src/views/404.hbs src/views/500.hbs
git commit -m "chore: wire web routes into the main app alongside the API"

git add README.md
git commit -m "docs: update README with full module list and business rules"
```

### 10.3 Crear el repositorio remoto y compartirlo con tu compañero

1. Entra a GitHub y crea un repositorio nuevo (por ejemplo `appcenar`), **sin** inicializarlo con README (ya tienes uno).
2. Conecta tu repo local con el remoto:

```bash
git branch -M main
git remote add origin https://github.com/TU_USUARIO/appcenar.git
git push -u origin main
```

3. Para compartirlo con tu compañero:
   - Ve a **Settings → Collaborators** en el repositorio de GitHub.
   - Haz clic en **Add people** y busca su usuario de GitHub o su correo.
   - Él aceptará la invitación por correo o desde su cuenta de GitHub, y ya podrá clonar y hacer push:

```bash
git clone https://github.com/TU_USUARIO/appcenar.git
```

4. Flujo de trabajo recomendado en equipo:
   - Crear una rama por funcionalidad: `git checkout -b feature/orders-module`
   - Hacer commits pequeños y descriptivos.
   - Subir la rama: `git push -u origin feature/orders-module`
   - Abrir un Pull Request hacia `main` para revisar el código antes de fusionar.

## 11. Requerimientos técnicos ya cubiertos en esta base
